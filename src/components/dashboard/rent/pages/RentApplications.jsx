...
const RentApplications = () => {
  ...
  const summaryStats = useMemo(() => ({
    ...
    pending: applications.filter(app => ['INSPECTION_SCHEDULED', 'INSPECTION_VERIFIED', 'APPLICATION_STARTED'].includes(app.status)).length,
    submitted: applications.filter(app => app.status === 'APPLICATION_SUBMITTED').length
  }), [applications]);
    ...
    <div className="rent-overview-container bg-gradient-to-br from-gray-50 to-gray-100 min-h-screen p-4 md:p-6 applications-page">
      <div className="bg-white rounded-lg shadow-md border border-[#e2e8f0] p-6 space-y-6 applications-card">
        ...
        <div className="grid gap-4 md:grid-cols-3">
          <div className="bg-white rounded-2xl shadow-sm border p-4" style={{ borderColor: 'var(--accent-color, #9F7539)' }}>
            <p className="text-xs uppercase tracking-wider" style={{ color: 'var(--accent-color, #9F7539)' }}>Applications</p>
            <p className="text-2xl font-bold" style={{ color: 'var(--primary-color, #0E1F42)' }}>{summaryStats.total}</p>
          </div>
          <div className="bg-white rounded-2xl shadow-sm border p-4" style={{ borderColor: 'var(--accent-color, #9F7539)' }}>
            <p className="text-xs uppercase tracking-wider" style={{ color: 'var(--accent-color, #9F7539)' }}>Actionable</p>
            <p className="text-2xl font-bold" style={{ color: 'var(--primary-color, #0E1F42)' }}>{summaryStats.pending}</p>
          </div>
          <div className="bg-white rounded-2xl shadow-sm border p-4" style={{ borderColor: 'var(--accent-color, #9F7539)' }}>
            <p className="text-xs uppercase tracking-wider" style={{ color: 'var(--accent-color, #9F7539)' }}>Submitted</p>
            <p className="text-2xl font-bold" style={{ color: 'var(--primary-color, #0E1F42)' }}>{summaryStats.submitted}</p>
          </div>
        </div>
      ...
      {modalType === 'missed' && selectedApplication && (
        <InspectionMissedModal
          application={selectedApplication}
          onReschedule={() => setModalType(null)}
          onCancel={() => updateApplication(selectedApplication.id, { status: 'CANCELLED' })}
          onClose={() => setModalType(null)}
        />
      )}
